...
const BarChartTesting: React.FC<BarChartTestingProps> = ({ chart }) => {
	...
	const initData = () => {
		...
		if (id.includes("intensitaet")) {
			...
			branchen.forEach((branche) => {
				const findValue = innoIntensitaet[branche.id];
				collectData.push({
					...branche,
					value: findValue,
				});
		...
		<section id={`treemap-${id}`} className="mb-20">
			...
			>
				{data.map((entry, index) => (
					...
					>
						<p className="bold mb-1 line-clamp-1">{entry.name}</p>
